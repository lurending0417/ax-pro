    ...
    <div>
        ...
        <TableCpn
            :heads="[
                ...
                {propName:'wareHouseName',name:'退货仓库'},
                {propName:'amount',name:'商品数量'},
                {propName:'refundCost',name:'退款金额', unit: '元'},
                {propName:'paid',name:'退款状态'},
                {propName:'salesmanName',name:'退货员工'},
                {tag:'BUTTON',name:'操作'}
            ]"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            add() {
                if (this.getRejectWareHouse.length === 0) {
                    alert('对不起，您所在的门店暂无回收仓库，不能新增退货单，请先添加回收仓')
                    return
                }
                this.clearData()
            ...
            initData() {
                this.getRejectWareHouse = baseUtil.getMetadataRejectWareHouse()
