        ...
        <div class="form-inline" style="margin-top: 20px">
            <div class="form-group form-group-margin">
                ...
                <select class="form-control" style="margin-left: 15px" @change="chooseWareHouse" v-model="wareHouseId$">
                    <option v-for="wareHouse in recycleWareHouse" :value="wareHouse.id">{{ wareHouse.name }}
                    </option>
                </select>
            </div>
        </div>
...
<script>
    ...
    import {calculateUtil, numberUtil, baseUtil} from '~/util';
    ...
    export default {
        ...
        mounted() {
            this.recycleWareHouse = baseUtil.getMetadataRejectWareHouse({enabled: true});
            this.wareHouseId$ = this.recycleWareHouse[0].id
        },
        beforeMount() {
        }
    }
