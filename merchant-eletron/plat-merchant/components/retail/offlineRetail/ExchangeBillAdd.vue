        ...
        <fieldset style="padding: 10px">
            ...
            <div class="form-inline">
                <div class="form-group form-group-margin">
                    ...
                    <select v-if="getCanChange" class="form-control" v-model="wareHouseId$">
                        <option v-for="wareHouse in rejectWareHouse" :value="wareHouse.id">{{ wareHouse.name }}
                        </option>
                    </select>
            ...
            <TableCpn
                ...
                :heads="[
                    ...
                    {propName:'roughWeight',name:'毛重', unit: 'g'},
                    {propName:'depreciationRate',name:'折旧率'},
                    {propName:'netWeight',name:'净重', unit: 'g'},
                    {propName:'processingCharges',name:'工费', unit: '元'},
                    {propName:'metalPrice',name:'金价', unit: '元'},
                    {propName:'originalPrice',name:'原价金额', unit: '元'},
                    {propName:'forfeit',name:'其他扣除', unit: '元'},
            ...
            <div class="col-md-9" style="text-align: right">
                ...
                <span v-else style="margin: 10px">
                    <button type="button" class="btn btn-sm btn-success" data-dismiss="modal"
                            @click="print(passed.wareBill, getExchangeBillDetailItems)">打印票据</button>
                </span>
...
<script>
    ...
    import { dateUtil, numberUtil, baseUtil } from '~/util'
    ...
    export default {
        ...
        methods: {
            ...
            print (wareBill, item) {
                ...
                let printTemplateChecked = 'EXCHANGE'
                if (templates[printTemplateChecked]) {
                    ...
                    item.map((subItem, index) => {
                        ...
                        content += ('来料金额：￥' + subItem.originalPrice + '\n');
                        content += ('其他扣除：￥' + (typeof (subItem.forfeit) === 'undefined' ? 0 : subItem.forfeit.toString()) + '\n');
                        content += ('实收金额：￥' + subItem.totalCost + '\n');
                        content += '\n';
                    });
                    DLL.TLXSetNamedVariableA(buffer.readInt32LE(0), TEcoding('来料详情'), TEcoding(content));
                    DLL.TLXSetNamedVariableA(buffer.readInt32LE(0), TEcoding('来料金额'), TEcoding('来料金额：￥' + numberUtil.toKeepTwoPlacesOfDecimalAndRound(acceptMount)));
                    DLL.TLXSetNamedVariableA(buffer.readInt32LE(0), TEcoding('扣除金额'), TEcoding('扣除金额：￥' + numberUtil.toKeepTwoPlacesOfDecimalAndRound(acceptMount - wareBill.totalCost)));
                    ...
                    DLL.TLXSetNamedVariableA(buffer.readInt32LE(0), TEcoding('操作人'), TEcoding('操作人：' + wareBill.creatorId));
            ...
            addItem() {
                this.exchangeBillItems.push({
                    ...
                    depreciationRate: 0,
                    netWeight: 0,
                    metalPrice: this.getMetalPrice.find(ele => ele.metalId === this.metals[0].id).retailPrice,
                    processingCharges: 0,
                    originalPrice: 0,
                    forfeit: 0,
                    totalCost: 0
