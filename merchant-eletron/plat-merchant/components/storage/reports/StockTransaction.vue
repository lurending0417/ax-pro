    ...
    <div>
        ...
        <table class="table table-bordered">
            ...
            <tr v-for="(item,index) in reportsData" :key="index">
                ...
                <td>{{handleItemUnit(item.retailFixedTotalCost, '元')}}</td>
                <td>{{handleItemUnit(item.retailFixedWeight, 'g')}}</td>
                <td>{{item.retailFixedAmount || '' }}</td>

                <td>{{handleItemUnit(item.retailWeightedProcessingCharges,'元') }}</td>
                <td>{{handleItemUnit(item.retailWeightedWeight,'g') }}</td>
                <td>{{item.retailWeightedAmount || ''}}</td>

                <td>{{handleItemUnit(item.exchangeFixedTotalCost,'元')}}</td>
                <td>{{handleItemUnit(item.exchangeFixedWeight,'g')}}</td>
                <td>{{item.exchangeFixedAmount || ''}}</td>

                <td>{{handleItemUnit(item.exchangeWeightedProcessingCharges, '元')}}</td>
                <td>{{handleItemUnit(item.exchangeWeightedWeight, 'g') }}</td>
                <td>{{item.exchangeWeightedAmount || '' }}</td>

                <td>{{handleItemUnit(item.leaseTotalCost,'元') }}</td>
                <td>{{handleItemUnit(item.leaseWeight, 'g') }}</td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            search(page) {
                let params = {...this.form, limit: 20, start: 0, storeId: this.storeId, sum: true}
                if (page) {
        ...
        mounted() {
            this.$watch('storeId', function (val) {
                this.wareHouses = baseUtil.getMetadataWareHouseByStoreId({storeId: val});
                this.refresh()
            })
