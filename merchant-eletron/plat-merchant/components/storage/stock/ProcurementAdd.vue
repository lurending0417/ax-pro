        ...
        <table class="table table-striped" style="background-color: rgb(221, 221, 221)">
            ...
            <tr v-for="(item, index) in stockItemsView" :class="{'tr-cls':item.isFocused}">
                ...
                <td class="table-bordered">
                    <input
                        v-if="getCanChange && showSerialInput"
                        type="text"
                        style="max-width: 100px"
                        @keyup.enter="enterSerial(item.id)"
                        @change="modifyItemHandler($event, item, 'serial')"
                ...
                <td class="table-bordered">
                    <input
                        ...
                        @focus="changeCurTrColor(index, item, 'weight')"
                        :ref="'weight' + item.id"
                        :value="item.weight"
                    >
...
//                    this.resetStockItems({index})
            ...
            changeCurTrColor(index, item, weight) {
...
//                    this.serialSource$ = preStockView.serialSource
...
//                    this.serialSource$ = constUtil._CONST_SERIAL_SOURCE.find(ele => ele.id === preStockView.serialSource).name
...
//                }
                ...
                this.showSerialInput = constUtil._CONST_SERIAL_SOURCE[1].id === _this.preStock.serialSource; // 当条码生成规则为手工生成时为true
            ...
            enterSerial(itemId) {
                if (this.showAmountField) {
                    this.$refs['amount' + itemId][0].select()
                } else {
                    ...
                    callback: (data) => {
                        ...
                        if (printTemplateChecked) {
                            if (templates[printTemplateChecked]) {
                                ...
                                if (data.rows[0].storage.mainGemWeight) {
                                    DLL.TLXSetNamedVariableA(buffer.readInt32LE(0), TEcoding('主石'), TEcoding(data.rows[0].storage.mainGemWeight + 'g / ' + data.rows[0].storage.mainGemAmount))
                                }
                                ...
                                DLL.TLXSetNamedVariableA(buffer.readInt32LE(0), TEcoding('总重量'), TEcoding(data.rows[0].storage.weight.toFixed(2) + data.rows[0].storage.weightUnit))
            ...
            clickToPort: function () {
                var serialport = requireFFI('serialport')
                ...
                this.port.on('data', data => {
                    ...
                    if (inputText.length === 14) {
                        console.log(inputText.substring(1, 9))
                        console.log(parseFloat(inputText.substring(1, 9)))
                        console.log('======')
                        console.log(this.itemId);
                        console.log(this.$refs['weight' + this.itemId])
                        if (this.$refs['weight' + this.itemId]) {
                            this.$refs['weight' + this.itemId][0].value = parseFloat(inputText.substring(1, 9))
                            let ev = document.createEvent('HTMLEvents');
                            ev.initEvent('change', false, true);
                            this.$refs['weight' + this.itemId][0].dispatchEvent(ev);
                        } else {
                            alert('请确认你是否选中重量输入框')
                        }
                        inputText = ''
...
//                    console.log('itemInfo', itemInfo);
            ...
            clickWeight (itemId) {
                console.log('weightweightweightweight')
                this.itemId = itemId
                console.log(this.itemId)
            }
        },
