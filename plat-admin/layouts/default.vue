    ...
    <el-container>
        ...
        <el-container>
            <el-header id="elHeader">
                <el-button type="info" plain :icon="toggleIcon" @click="toggleMenu"></el-button>
                <div class="profile">
                    <el-button type="default" class="no-outline">快捷菜单1</el-button>
                    <div style="display: inline; padding-left: 10px;">
                        <el-dropdown @command="dropDown">
                            <el-button type="default" class="no-outline">
                        ...
                        <span class="avatar">
                            <img v-if="avatar" :src="renderResource(avatar,'')">
                        </span>
                    </div>
                </div>
            </el-header>
...
<script>
    ...
    export default {
        ...
        computed: {
            name: () => {
                let info = baseUtil.getBasicInfo();
                let ret = "";
                if (info.platEmployee) {
                    ret = info.platEmployee.name;
                }
                if (!ret && info.user) {
                    ret = info.user.nickName;
                    if (!ret) {
                        ret = info.user.telephone;
                    }
                }
                return ret + '';
            },
            avatar: () => {
                let info = baseUtil.getBasicInfo();
                if (info && info.user) {
                    return info.user.avatarPath;
                }
            }
        },
        beforeCreate() {
            if (!authUtil.isAuthorized(this.$router.currentRoute.fullPath)) {
                this.$router.push("/");
            }
        },
        mounted() {
        ...
        methods: {
            syncHeight() {
                ...
                var headerHeight = document.getElementById("elHeader").clientHeight;
                var padding = window.getComputedStyle(document.getElementById("elMain")), paddingTop = parseInt(padding.paddingTop), paddingBottom = parseInt(padding.paddingBottom);
                var height = docHeight - headerHeight - paddingTop - paddingBottom;
