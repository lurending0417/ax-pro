<template>
    <div>
        ...
        <el-row type="flex" justify="space-between" class="m-t">
            <el-col :span="12">
                <el-card class="box-card">
                    <div slot="header">
                        <span><span class="text-danger text-bold">平台</span>角色树</span>
                    </div>
                    <el-tree
                        :data="treeNodes"
                        highlight-current
            ...
            <el-col :span="12">
                <el-card class="box-card m-l">
                    <div slot="header">
                        <span><span class="text-danger text-bold">平台</span>页面功能树</span>
                    </div>
                    <el-tree
        ...
        <AdminRoleGroupEditor :options="groupModal" @close="closeRoleGroupEditor"></AdminRoleGroupEditor>

        <AdminRoleEditor :options="roleModal" @close="closeRoleEditor"></AdminRoleEditor>
...
<script>
    ...
    export default {
        ...
        methods: {
            roleTree() {
                api.request({
                    url: api.url.auth.admin.role.tree,
                    success: (json) => {
            ...
            deleteRoleGroup() {
                this.$confirm('您确定要删除选中的角色分组吗？', '提示', {
                    ...
                    api.request({
                        url: api.url.auth.admin.role.group.del,
                        params: {
            ...
            deleteRole() {
                this.$confirm('您确定要删除选中的角色吗？', '提示', {
                    ...
                    api.request({
                        url: api.url.auth.admin.role.del,
                        params: {
                            roleId: this.currentNode.id
                        },
                        success: () => {
                            this.$message.success('删除成功！');
                            this.roleTree();
            ...
            actionTree(roleId) {
                ...
                api.request({
                    url: api.url.auth.admin.role.action.tree,
                    params: {
                        roleId: roleId
                    },
                    success: (json) => {
            ...
            setRolePageAction: lodash.debounce(function () {
                ...
                api.request({
                    url: api.url.auth.admin.role.action.add,
                    params: {
                        roleId: this.currentNode.id,
