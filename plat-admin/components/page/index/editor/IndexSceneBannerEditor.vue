    ...
    <el-dialog
        ...
        <el-form :model="params" label-width="80px">
            ...
            <el-form-item label="图片文件">
                <el-upload ref="upload" :file-list="files" :action="reqUrl" :auto-upload="false" :data="params" :on-success="success" :on-error="error" :on-change="fileChange" :before-remove="fileChange" :multiple="true" :show-file-list="true" :limit="limit">
        ...
        watch: {
            'options.visible': function (value) {
                ...
                if (this.options.action === 'add') {
                    this.reqUrl = api.url.page.index.scene.banner.add.toString();
                } else {
                    this.reqUrl = api.url.page.index.scene.banner.edit.toString();
                }
                this.files = [];
            }
        },
        methods: {
            ...
            submit() {
                if (this.options.action === 'edit') {
                    this.params.bannerId = this.params.id;
                }
