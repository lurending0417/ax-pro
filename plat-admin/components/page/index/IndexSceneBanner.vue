<template>
    <el-dialog
        ...
        <IndexSceneBannerEditor :options="editorModal" @close="closeEditor"></IndexSceneBannerEditor>
...
<script>
    ...
    export default {
        ...
        methods: {
            list() {
                api.request({
                    url: api.url.page.index.scene.banner.list,
                    params: this.params,
                    success: (json) => {
                        this.rows = json.rows;
            ...
            deleteBanner(row) {
                this.$confirm('您确定要删除选中的BANNER吗？', '提示', {
                    ...
                    api.request({
                        url: api.url.page.index.scene.banner.del,
                        params: {
                            bannerId: row.id
                        },
                        success: () => {
