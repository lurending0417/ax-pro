            ...
            <div class="form-inline">
                <div class="form-group form-group-margin">
                    ...
                    <select v-model="wareHouseId$" class="form-control" style="margin-left: 15px" :disabled="getWareItems.length>=1">
                        <option v-for="wareHouse in getWareHouses" :value="wareHouse.id">{{ wareHouse.name }}
                        </option>
                    </select>
                </div>
                <div class="form-group form-group-margin">
                ...
                <div class="form-group form-group-margin" v-for="(person, index) in staffArr">
                    <label>销售员</label>
                    <select v-model="person.staffId" class="form-control" style="margin-left: 15px">
                        <option v-for="staff in getStaffs" :value="staff.id">{{ staff.nickName }}
                        </option>
                    </select>
                </div>
        ...
        <div class="row" style="margin-bottom: 20px">
            <div class="col-md-12" style="text-align: right">
                <span v-if="getCanChange">
                    <button type="button" class="btn btn-sm btn-default" @click="clickAddRetailBill">保存并提交</button>
                    <button type="button" style="margin: 10px" class="btn btn-sm btn-default"
                            @click="jump2payment">前往收银</button>
                </span>
                <span v-else>
                    <button type="button" style="margin: 10px" class="btn btn-sm btn-primary" data-dismiss="modal"
                ...
                <span>
                    <button type="button" class="btn btn-sm btn-default" @click="back">返回</button>
                </span>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 打印标签
            print(bill, billItems) {
                console.log(bill)
                console.log(billItems)
                console.log('打印标签ing:', billItems)
                console.log('=====')
            },
            jump2payment() {
            ...
            clickPlus() {
                ...
                console.log(len, this.getStaffs.length)
                if (len < this.getStaffs.length) {
                    this.staffArr.push({
            ...
            initData() {
                console.log('新增零售单初始化页面');
                this.apiGetMetalPrice() // 重置所有金价
                this.wareHouseId$ = this.getWareHouses[0].id
//                this.staffArr[0].staffId = this.getStaffs[0].id
