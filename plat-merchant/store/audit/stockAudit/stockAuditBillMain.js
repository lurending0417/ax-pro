        ...
        stocksView: state => { // 入库审批单详情
            state.stockAuditBillList.map(bill => {
                ...
                if (bill.type === 'GOODS_STOCK') {
                    ...
                    item.buyerName = baseUtil.getMetadataStaffById(item.buyerId).nickName
                    ...
                    item.status = baseUtil.getInventoryBillStatu(item.status)
                    item.creatorId = baseUtil.getNameFromPropId('staffs', item.creatorId)
                }
                item.nodeUserName = baseUtil.getMetadataStaffById(bill.nodeUserId).nickName
