import api from '~/api'
import {dateUtil, baseUtil, objectUtil, constUtil} from '~/util'
...
export default {
    ...
    getters: {
        ...
        getTotalPay: state => {
            console.log(typeof state.totalPay)
    ...
    actions: {
        ...
        getRetailRecycleBillDetail({commit, state}, obj) {
            ...
            api.request({
                ...
                success(data) {
                    console.log('获取回收单详情', data)
                    let totalPay = 0;
                    data.rows.forEach((item) => {
                        totalPay += item.totalCost
                    });
                    commit('resetTotalPay', totalPay);
