...
export default {
    ...
    getters: {
        ...
        getWareHouses: () => {
            let curWareHouse = baseUtil.getMetadataWareHouseByStoreId(baseUtil.getStore().id) // 当前门店所有仓库
            return curWareHouse.filter(item => !item.recycle)
        },
        getStaffs: () => {
            return baseUtil.getMetadataStaffByStoreId(baseUtil.getStore().id)
        },
        getWareItems: state => {
            console.log('getWareItems')
    ...
    mutations: {
        ...
        resetAllWareItems(state, wareItems) {
            console.log('resetAllWareItems', wareItems)
