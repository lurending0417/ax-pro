<template>
	<el-dialog append-to-body
		...
		<el-row class="row m-b-m" type="flex">
			<span class="m-r-m"><strong>大师昵称：</strong>{{options.row.nick}}</span>
			<span class="m-r-m"><strong>大师手机：</strong>{{options.row.phone}}</span>
			<span class="m-r-m"><strong>类型：</strong>{{options.row.categoryName}}</span>
		</el-row>
...
<script>
	...
	export default {
		...
		methods: {
			save() {
				api.request({
					url: api.url.page.index.master.edit,
					params: {...this.form},
					success: json => {
						this.close('refresh')
					}
